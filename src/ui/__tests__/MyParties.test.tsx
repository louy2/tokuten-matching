...
describe("MyParties — no character filter (moved to PartyList)", () => {
  ...
  it("does not render character filter buttons", async () => {
    setupFetchMock(mockUser, { "/api/my-parties": partiesWithClaims });
    renderWithProviders(<MyParties />);
    await waitFor(() => {
      expect(screen.getByText("Party A")).toBeInTheDocument();
    });
    expect(screen.queryByRole("button", { name: "Ayumu Uehara" })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Kasumi Nakasu" })).not.toBeInTheDocument();
  });

  it("does not render 'From Profile' button", async () => {
